    ...
    return (
        <>
            <div className="fixed top-0 left-0 bg-white w-[72px] gap-6 flex items-center pt-3 flex-col border-r-[1.3px] h-screen border-[#CFD1D4]">
                ...
                <div className="" >
                    <div onClick={() => setColorToggle(true)} className="cursor-pointer">
                        <div className={`w-[40px] h-[40px] ${colorToggle ? "bg-primary" : "bg-white "}  mx-auto rounded-lg flex items-center justify-center`}>
                            <AiFillAppstore className={` ${colorToggle ? "text-white" : "text-[#6A6F79] "}  text-2xl`} />
                        </div>
                        <p className={`text-center text-sm font-semibold mt-0.5  ${colorToggle ? "text-primary" : "text-[#6A6F79] "}`}>Dashbo...</p>
                    </div>
                    <div className="w-full border-b mt-2 border-[#EAEAEA]"></div>
                </div>
                <div onClick={() => setColorToggle(false)} className={` cursor-pointer ${!colorToggle ? "text-white" : "text-[#6A6F79]"}`}>
                    <div className={`w-[40px] ${!colorToggle ? "bg-primary" : "bg-white "} h-[40px]  mx-auto rounded-lg flex items-center justify-center`}>
                        <MdPeopleAlt className=' text-2xl' />
                    </div>
                    <p className={`${!colorToggle ? "text-primary mt-0.5" : "text-[#6A6F79] "} text-center  text-sm font-semibold `}>Patient</p>
                </div>
            </div>
